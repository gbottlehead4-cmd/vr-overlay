 ...
 * `VISORVR_DEFINE_SPARSE_JSON(T, ...)`
 * ------------------------------------------
 *
 * Similar to `VISORVR_DEFINE_JSON`, but it skips keys that match the
 ...
 *
 *
 * `VISORVR_DECLARE_SPARSE_JSON(T)`
 * --------------------------------------
 *
 * From `<VisorVR/json_fwd.hpp>`, optional forward declarations for
...
#define DETAIL_VISORVR_FROM_JSON_DECLARE_KEY(KeyTransform) \
  constexpr auto ok_json_field_key_##KeyTransform = \
    detail::SparseJson::ConstStr##KeyTransform(wrapped);
#define DETAIL_VISORVR_FROM_JSON_TRY_KEY(v1, KeyTransform) \
...
#define DETAIL_VISORVR_FROM_JSON(v1) \
  { \
    constexpr auto wrapped = detail::SparseJson::Wrap(#v1); \
    DETAIL_VISORVR_FROM_JSON_DECLARE_KEY(SkipFirst) \
    DETAIL_VISORVR_FROM_JSON_DECLARE_KEY(SkipFirstLowerNext) \
    DETAIL_VISORVR_FROM_JSON_TRY_KEY(v1, SkipFirst) \
    else DETAIL_VISORVR_FROM_JSON_TRY_KEY(v1, SkipFirstLowerNext) \
  }

#define DETAIL_VISORVR_TO_SPARSE_JSON(v1) \
...
// but `FOO(VISORVR_IDVA(__VA_ARGS__))` will pass as multiple
// arguments
#define VISORVR_IDVA(...) __VA_ARGS__

#define VISORVR_DEFINE_SPARSE_JSON_DETAILS(T) \
  VISORVR_DECLARE_SPARSE_JSON(T) \
  template <> \
...
#define VISORVR_DEFINE_FROM_JSON(T, ...) \
  void from_json(const nlohmann::json& nlohmann_json_j, T& nlohmann_json_v) { \
    NLOHMANN_JSON_EXPAND(NLOHMANN_JSON_PASTE( \
      DETAIL_VISORVR_FROM_JSON, VISORVR_IDVA(__VA_ARGS__))) \
    from_json_postprocess<T>(nlohmann_json_j, nlohmann_json_v); \
  }

#define VISORVR_DEFINE_SPARSE_JSON_TO_JSON_WITH_DEFAULT(T, ...) \
  template <> \
  void to_json_with_default<T>( \
    ...
    NLOHMANN_JSON_EXPAND(NLOHMANN_JSON_PASTE( \
      DETAIL_VISORVR_TO_SPARSE_JSON, VISORVR_IDVA(__VA_ARGS__))) \
    to_json_postprocess<T>( \
      nlohmann_json_j, nlohmann_json_default_object, nlohmann_json_v); \
  }

#define VISORVR_DEFINE_SPARSE_JSON_TO_JSON(T) \
...
#define VISORVR_DEFINE_SPARSE_JSON(T, ...) \
  VISORVR_DEFINE_SPARSE_JSON_DETAILS(T) \
  VISORVR_DEFINE_FROM_JSON(T, VISORVR_IDVA(__VA_ARGS__)) \
  VISORVR_DEFINE_SPARSE_JSON_TO_JSON_WITH_DEFAULT( \
    T, VISORVR_IDVA(__VA_ARGS__)) \
  VISORVR_DEFINE_SPARSE_JSON_TO_JSON(T)

#define VISORVR_DEFINE_JSON_DETAILS(T) \
  VISORVR_DECLARE_JSON(T) \
  template <> \
  struct SparseJsonDetail<T> { \
    static inline void \
    Encode(nlohmann::json& j, const T& /*parent_v*/, const T& v) { \
...
#define DETAIL_VISORVR_TO_JSON(v1) \
  constexpr auto ok_json_field_key_##v1 = \
    detail::SparseJson::ConstStrSkipFirst(detail::SparseJson::Wrap(#v1)); \
  SparseJsonDetail<decltype(nlohmann_json_v.v1)>::Encode( \
    nlohmann_json_j[ok_json_field_key_##v1], {}, nlohmann_json_v.v1);
#define VISORVR_DEFINE_TO_JSON(T, ...) \
  void to_json(nlohmann::json& nlohmann_json_j, const T& nlohmann_json_v) { \
    NLOHMANN_JSON_EXPAND(NLOHMANN_JSON_PASTE( \
      DETAIL_VISORVR_TO_JSON, VISORVR_IDVA(__VA_ARGS__))) \
    to_json_postprocess<T>(nlohmann_json_j, nlohmann_json_v); \
  }
#define VISORVR_DEFINE_JSON(T, ...) \
  VISORVR_DEFINE_JSON_DETAILS(T) \
  VISORVR_DEFINE_FROM_JSON(T, VISORVR_IDVA(__VA_ARGS__)) \
  VISORVR_DEFINE_TO_JSON(T, VISORVR_IDVA(__VA_ARGS__))
