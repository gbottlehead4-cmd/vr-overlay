...
#include <VisorVR/format/enum.hpp>
...
struct ThreadPoolAwaitable {
  ...
  auto await_resume() const {
    ...
    VISORVR_ASSERT(mResult != Derived::pending_result);
    return mResult;
  }

  void await_suspend(std::coroutine_handle<> coro) {
    VISORVR_ASSERT(!mCoro, "await_suspend called twice");
    mCoro = coro;
    ...
    VISORVR_ASSERT(transitioned.error() == State::Canceling);
    mResult = Derived::canceled_result;
