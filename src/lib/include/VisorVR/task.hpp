...
#ifndef VISORVR_TASK_TRACE
#define VISORVR_TASK_TRACE(...)
#endif
...
struct TaskState {
  ...
  explicit TaskState(
    ...
    VISORVR_ASSERT(initialOwner == TaskStateOwner::Producer);
  }
  ...
  constexpr TaskAwaitReady await_ready() const noexcept {
    if ((mRefBits & ThreadPoolBit)) {
      VISORVR_TASK_TRACE("TaskState::await_ready/threadPool");
    ...
    if ((mRefBits & ProducerBit)) {
      VISORVR_TASK_TRACE("TaskState::await_ready/producer");
      return TaskAwaitReady::NotReady;
    }
    ...
    if constexpr (
      ...
      VISORVR_TASK_TRACE("TaskState::await_ready/anyThread");
      return TaskAwaitReady::Ready;
    } else {
      if (mContext.is_same_com_context()) {
        VISORVR_TASK_TRACE("TaskState::await_ready/ready");
        return TaskAwaitReady::Ready;
      }
      VISORVR_TASK_TRACE("TaskState::await_ready/readyWrongThread");
      return TaskAwaitReady::ReadyWrongThread;
    }
  }

  void return_void() {
    VISORVR_ASSERT(holds_alternative<std::monostate>(mResult));
  ...
  [[nodiscard]] bool release() noexcept {
    ...
    VISORVR_ASSERT((mRefBits & bits) == bits);
    if constexpr (bits & ThreadPoolBit) {
      VISORVR_ASSERT(!(mRefBits & ProducerBit));
    }
    mRefBits &= ~bits;
    VISORVR_TASK_TRACE(
...
struct TaskFinalAwaiter {
  ...
  std::coroutine_handle<> await_suspend(
    ...
    VISORVR_TASK_TRACE("TaskFinalAwaiter::await_suspend");
    auto state = std::move(producer.promise().mState);

    VISORVR_TASK_TRACE("TaskFinalAwaiter::await_suspend/acquiringLock");
    auto lock = state.lock();
    VISORVR_TASK_TRACE("TaskFinalAwaiter::await_suspend/locked");
    const auto handle = lock->mConsumerHandle;
    const auto context = lock->get_task_context();
    VISORVR_TASK_TRACE("TaskFinalAwaiter::await_suspend/unlocked");
    ...
    if (!handle) {
      VISORVR_TASK_TRACE("TaskFinalAwaiter::await_suspend/nullHandle");
    ...
    if constexpr (
      TTraits::CompletionThread == TaskCompletionThread::AnyThread) {
      VISORVR_TASK_TRACE("TaskFinalAwaiter::await_suspend/anyThread");
      ...
      if (context.is_same_com_context()) {
        VISORVR_TASK_TRACE("TaskFinalAwaiter::await_suspend/thisThread");
        return [](auto toDestroy, auto toResume) -> DetachedTask {
          toDestroy.destroy();
          toResume.resume();
          co_return;
      ...
      VISORVR_TASK_TRACE("TaskFinalAwaiter::await_suspend/wrongThread");
...
struct TaskPromiseBase {
  ...
  auto get_return_object() {
    VISORVR_TASK_TRACE("TaskPromiseBase::get_return_object");
...
struct TaskAwaiter {
  ...
  bool await_ready() const noexcept {
    VISORVR_ASSERT(mState, "Can't await a null promise");
    return mState.lock()->await_ready() == TaskAwaitReady::Ready;
  }

  std::coroutine_handle<> await_suspend(std::coroutine_handle<> caller) {
    VISORVR_TASK_TRACE("TaskAwaiter::await_suspend");
    VISORVR_ASSERT(mState, "Can't await a null promise");
    VISORVR_TASK_TRACE("TaskAwaiter::await_suspend/locking");
    auto lock = mState.lock();
    VISORVR_TASK_TRACE("TaskAwaiter::await_suspend/locked");
    ...
    switch (lock->await_ready()) {
      case Ready:
        VISORVR_TASK_TRACE("TaskAwaiter::await_suspend/ready");
        return caller;
      case NotReady:
        VISORVR_TASK_TRACE("TaskAwaiter::await_suspend/notReady");
        lock->mConsumerHandle = caller;
        return std::noop_coroutine();
      case ReadyWrongThread:
        VISORVR_TASK_TRACE("TaskAwaiter::await_suspend/readyWrongThread");
        lock->mConsumerHandle = caller;
        VISORVR_ASSERT(caller);
  ...
  auto await_resume() {
    ...
    VISORVR_ASSERT(state->await_ready() == TaskAwaitReady::Ready);
...
namespace this_task {
/** Call `VisorVR::fatal()` if this task throws an exception.
 *
