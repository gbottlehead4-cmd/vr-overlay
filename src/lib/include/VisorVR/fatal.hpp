...
#define VISORVR_ALWAYS_ASSERT(x, ...) \
  ::VisorVR::detail::assert_impl(static_cast<bool>(x), #x, ##__VA_ARGS__);

// Maybe change this to depend on DEBUG in the future
#define VISORVR_ASSERT(x, ...) \
  VISORVR_ALWAYS_ASSERT(x, ##__VA_ARGS__)
