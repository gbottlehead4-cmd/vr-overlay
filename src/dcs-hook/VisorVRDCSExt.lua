...
local status, VisorVR = pcall(require, "VisorVR_LuaAPI64")
if not status then
  return
end
VisorVR.EventPrefix = "dcs/"

local Terrain = require('terrain')

function VisorVR.send(name, value)
  VisorVR.sendRaw(VisorVR.EventPrefix..name, value)
end
function VisorVR.sendMulti(events)
  local prefixed = {}
  for k,event in ipairs(events) do
    local name, value = unpack(event)
    table.insert(prefixed, { VisorVR.EventPrefix..name, value })
  end
  VisorVR.sendRaw(
    "MultiEvent",
...
function callbacks.onSimulationStart()
  ...
  if not selfData then
    VisorVR.send("SimulationStart", startData);
    sendState()
    return
  end
