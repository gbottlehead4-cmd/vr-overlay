...
[assembly: InternalsVisibleTo(assemblyName: "VisorVR-Installer.aot")] // assembly name + '.aot suffix
...
async Task<int> CreateInstaller(DirectoryInfo inputRoot, string? signingKeyId, string? timestampServer,
    FileInfo? stampFile)
{
    if (!System.IO.File.Exists($"{inputRoot}/bin/VisorVRApp.exe"))
    {
        Console.WriteLine($"Cannot find bin/VisorVRApp.exe in INPUT_ROOT ({inputRoot.FullName})");
        return 1;
    }
    ...
    project.AddAction(
        new BinaryFileAction(GetMsixRemovalToolId().Value, "Removing legacy MSIX versions of VisorVR...",
...
{
    return new Id("Binary_remove_visorvr_msix_exe");
}

Id GetExecutableId()
{
    return new Id("File_Bin_VisorVRApp_exe");
}
...
ManagedProject CreateProject(DirectoryInfo inputRoot)
{
    ...
    var project =
        new ManagedProject("VisorVR",
            new Dir(@"%ProgramFiles%\VisorVR",
                new Dir("bin",
                    new File(GetExecutableId(), "bin/VisorVRApp.exe"),
                    new Files("bin/*.*", f => !f.EndsWith("VisorVRApp.exe"))),
            ...
            new Binary(GetMsixRemovalToolId(), "installer/remove-visorvr-msix.exe"));
    project.AddProperty(new Property("VVR_HARDLINKS", hardLinks.Select(l => l.ToString()).JoinBy(",")));
    ...
    project.UpgradeCode = Guid.Parse("e8647307-428a-4ea8-b033-473cc869e27e");
    project.ProductId = Guid.NewGuid();
    project.Platform = Platform.x64;

    project.ControlPanelInfo.Manufacturer = "VisorVR";
    ...
    project.AddRegValue(new RegValue(RegistryHive.LocalMachine, @"SOFTWARE\VisorVR", "InstallDir",
        "[INSTALLDIR]"));
    project.AddRegValue(new RegValue(RegistryHive.LocalMachine, @"SOFTWARE\VisorVR\Installer",
...
void RegisterApiLayers(DirectoryInfo directoryInfo, ManagedProject managedProject)
{
    ...
    managedProject.AddRegValue(new RegValue(RegistryHive.LocalMachine, apiLayersKey,
        $"[INSTALLDIR]bin\\VisorVR-OpenXR.json", 0));
    var value = new RegValue(RegistryHive.LocalMachine, apiLayersKey,
        $"[INSTALLDIR]bin\\VisorVR-OpenXR32.json", 0);
    value.Win64 = false;
...
void DisableLegacyApiLayers(SetupEventArgs args)
{
    ...
    var installedPath = Path.GetFullPath(args.InstallDir + @"\bin\VisorVR-OpenXR.json");
...
void CreateShortcuts(DirectoryInfo directoryInfo, ManagedProject managedProject)
{
    ...
    file.AddShortcuts(
        new FileShortcut("VisorVR", "INSTALLDIR"),
        new FileShortcut("VisorVR", "%Desktop%"),
        new FileShortcut("VisorVR", "%ProgramMenuFolder%"));
}
...
public class CustomActions
{
    ...
    {
        const string RegKeyPath = @"SOFTWARE\VisorVR\Installer\HardLinks";
        string rawLinks = session.Property("VVR_HARDLINKS");
